...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed(controls[4]) and current_state == ControlState.ACTIVE:
		attack()
	if dash_ability and dash_value == 0 and Input.is_action_just_pressed(controls[5]) and current_state == ControlState.ACTIVE:
